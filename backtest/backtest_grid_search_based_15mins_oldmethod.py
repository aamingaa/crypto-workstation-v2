...
# sharpe PNL sortino(半波动度)
# 时间——唯快不破（刚刚好），过程中-稳定+精准，

# =======仓位，10%，重仓赶紧去；0.5%，仓位轻一些，-0.2%，-10%，重仓做空，
...
def generate_etime_close_data_divd_time(bgn_date, end_date, index_code, frequency): # 2022-11-25 edition last edited on 2023-01-17
    ...
    # read_file_path = 'D:/9_quant_course/commodities_data/' + index_code + '_' + frequency + '.xlsx'
...
def backtest(original_data, index_code, frequency, n_days):
    ...
    # final_frame['ret'] = (final_frame['close'].shift(-t_delta) / final_frame['close'] - 1).fillna(0.0)
        # 可优化
    ...
    # 初始化线性回归模型
    model = LinearRegression(fit_intercept=True) # 极简主义，保证因子的原汁原味的成绩 weight,bias-tvalue f-value--线性模型 layer batch_norm

    # 划分训练集、测试集
    X_train = data_for_model.loc[: train_set_end_index, 'fct'].values.reshape(-1, 1) # reshape成为一列 [[1], [2], [3]] (n,1) [1,2,3,4,5] (5,)
    ...
    # 向量化的写法：
    # clip_num = 2
    # ret_frame_train_total['position'] = ret_frame_train_total['position'].clip(-clip_num, clip_num)
    #作业2：  clip() 理解这里的写法
    ...
    for i in range(0, len(ret_frame_train), 1):
        ...
        if i == 0 or ret_frame_train.loc[i - 1, 'position'] == 0:  # 如果是第一个时间步或前一个区间的结束时刻为空仓状态
            ...
            ret = close_2 / close_1 - 1 # 注意这里用的是算数收益率而不是log收益率
            ...
            if ret_frame_train.loc[i - 1, 'position'] > 0:  # 如果上一周期开的是多仓 之前是i-1，暂时删除试试
                ...
                ret_frame_train.loc[i, '持仓净值'] = 1 + ret * position 
            elif ret_frame_train.loc[i - 1, 'position'] < 0:  # 如果上一周期开的是空仓
                # ret_frame_train.loc[i, '持仓净值'] = 1.0 * (2 - close_2 / close_1)
                ret_frame_train.loc[i, '持仓净值'] = 1 - ret * position
    ...
    # ret_frame_train['change_pct'] = ret_frame_train.close.pct_change(1).fillna(0)
    # ret_frame_train['position'] = ret_frame_train['position'].shift(1) 
    # ret_frame_train['last_pos'] = ret_frame_train['position'].shift(1)
    # ret_frame_train['inc_pos'] = ret_frame_train['position'] - ret_frame_train['last_pos']
    # ret_frame_train = ret_frame_train.replace([np.inf, -np.inf, np.nan], 0.0)
    # fee_rate = 0.001
    # ret_frame_train['持仓净值（累计）'] = 1 + ((ret_frame_train.last_pos + ret_frame_train.inc_pos) * ret_frame_train.change_pct - fee_rate * ret_frame_train.inc_pos.abs()).cumsum()
    # ret_frame_train['持仓净值'] = ret_frame_train['持仓净值（累计）'].pct_change(1).fillna(0)
    ...
    for i in range(0, len(ret_frame_train_test), 1):
        ...
        if i == 0 or ret_frame_train_test.loc[i - 1, 'position'] == 0:  # 如果是第一个时间步或前一个区间的结束时刻为空仓状态
            ...
            if ret_frame_train_test.loc[i - 1, 'position'] > 0:  # 如果上一周期开的是多仓
                ...
                ret_frame_train_test.loc[i, '持仓净值'] = 1 + ret * position
            elif ret_frame_train_test.loc[i - 1, 'position'] < 0:  # 如果上一周期开的是空仓
                # ret_frame_train_test.loc[i, '持仓净值'] = 1.0 * (2 - close_2 / close_1)
                ret_frame_train_test.loc[i, '持仓净值'] = 1 + ret * position
